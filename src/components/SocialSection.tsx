 ...
 * Iconos con efecto glow dorado, animaciones escalonadas.
 */
...
const SocialSection = () => {
  ...
  return (
    <section className="py-14 md:py-24 bg-gradient-to-b from-background to-secondary/20" ref={referencia}>
      <div className="container mx-auto px-6 text-center">
        {/* ── Encabezado ── */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={enVista ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="font-body text-sm md:text-lg tracking-[0.3em] uppercase text-gold mb-3">Síguenos</p>
          <h2 className="font-display text-2xl md:text-4xl font-semibold text-foreground mb-8 md:mb-14">
        ...
        <div className="flex justify-center gap-6 md:gap-16">
          {redesSociales.map((red, indice) => {
            ...
            return (
              ...
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-accent border border-border/50 flex items-center justify-center group-hover:bg-gold/10 group-hover:border-gold/40 group-hover:shadow-[0_0_20px_hsl(38_65%_50%/0.15)] transition-all duration-300">
                  <Icono className="w-7 h-7 md:w-9 md:h-9 text-muted-foreground group-hover:text-gold transition-colors duration-300" />
                </div>
                <span className="font-display text-xs md:text-sm font-medium text-muted-foreground group-hover:text-foreground transition-colors duration-300">
                  {red.nombre}
                </span>
              </motion.a>
            );
